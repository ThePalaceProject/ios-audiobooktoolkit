    ...
    for (manifestJSON, expectedTitle) in expectedFirstChapterTitles {
      do {
        ...
        XCTAssertEqual(
          firstChapterTitle,
          expectedTitle,
          "Expected first chapter title to be \"\(expectedTitle)\" in \(manifestJSON.rawValue), but found \"\(firstChapterTitle)\""
        )
      } catch {
        XCTFail("Failed loading \(manifestJSON.rawValue) with error: \(error)")
      }
        ...
        for (index, expectedDuration) in manifestJSON.chapterDurations.enumerated() {
          ...
          if let duration = tocItem.duration {
            XCTAssertEqual(
              ...
              expectedDuration,
              accuracy: 0.1,
              "Chapter \(index + 1) duration mismatch in \(manifestJSON.rawValue)."
            )
          } else {
