...
| 21 | **`preferChapterEndingHere` is live debt across six call sites.** PP-4948 corrected which chapter a position exactly on a boundary belongs to, but the six resolution sites feeding the `.completed(chapter)` playback signal are pinned to the OLD tie-break, because flipping them changes playback control: `.completed` reaches `handlePlaybackCompleted`, which saves the position and pauses. Measured: without the pin the reported chapter changes for 9 of 10 chapters on `secret_lives_manifest`. The parameter exists to keep the display fix and the playback-control decision independent, and should not outlive that decision — if it does, make it a two-case enum rather than a boolean. **The premise differs by playback path, and an earlier version of this row flattened it.** On the AVPlayer paths `.completed` never fires mid-book; on **Findaway it fires at every chapter and always has** (`FAEPlaybackChapterComplete`, with `FAEPlaybackAudiobookComplete` a separate end-of-book event). So on shipping DRM titles the pin PRESERVES a live defect rather than avoiding one: the old tie-break names the chapter BEFORE the one that finished, so `saveLocation` stores the previous chapter's start and the patron's place moves backwards at every chapter boundary. **Flipping the flag halves that, it does not fix it** — `handlePlaybackCompleted` saves `chapter.position`, the chapter START, so completing chapter N moves the save from "start of N−1" to "start of N"; the residual rewind is in WHAT is saved, and the periodic timer only publishes `.positionUpdated`, never `saveLocation`. **And the pause is not AVPlayer-only or prospective**: the app's `.playbackCompleted` handler is player-agnostic, so Findaway pauses per chapter today, masked by the following `.playbackBegan` — an ordering assumption, not a guarantee, given this repo documents the Findaway chapter notification as arriving seconds late. The flip would not change WHETHER `.completed` fires on Findaway (notification-driven), only which chapter it names. Also: `SleepTimer` arms its target from `currentChapter` (unpinned, new tie-break) while `.completed` carries the old one, so the two disagree at a boundary — its `.completed`-driven trigger REMAINS dead mid-book, as it was on `origin/main`, and the feature still works via the polling fallback (`currentChapter != target` -> sleep within ~1s). Dead code, not a dead feature, and not newly caused | review, PP-4948 | **ticketed (PP-4951)** |
