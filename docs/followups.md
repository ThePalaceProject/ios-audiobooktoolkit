...
| 16 | `OverdriveTrack` duration: the **race** is fixed (lock-guarded value, atomic claim) but the first attempt at the **reload storm** did not hold — `localDirectory()` returns a path whether or not a file exists, so pre-download every read still claimed, failed and released. Now gated on `fileExists` plus a bounded `.failed(attempts:)` state, and a successful-but-unusable load (NaN/0) no longer latches `.resolved`. Still untested: no test anywhere references `OverdriveTrack`. Needs an injected loader seam to count invocations | review, wave 3 | open — code fixed, unpinned by tests |
...
| H | ~~`OverdriveTrack` duration race + unbounded reload storm — "fixed in wave 3"~~ **REOPENED as item 16.** The race was fixed; the storm was not. Closing it was premature and is recorded here rather than quietly edited, because a closed row that was never true is worse than an open one | review, wave 3 | see item 16 |
| I | Setting pinned target-level only, so a future target would not inherit | fixed — project-level too |
| J | `ConcurrencySafetyPrimitivesTests` mutates a process-global resolver 500×, unsafe once parallel testing is on | fixed in wave 3 — scheme pinned `parallelizable="NO"` |
