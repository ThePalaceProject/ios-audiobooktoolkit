...
| 18 | `fileExists` is not "the file is complete". `MediaProcessor.optimizeQTFile`, which the shared download delegate reaches through `verifyDownloadAndMove`, creates a **zero-byte file at the final url** and then writes it progressively, so every existence check passes throughout the write. `OverdriveTrack` now tolerates this (a superseded load is re-read once the download reports completion), but nothing else that gates on `fileExists` has been audited against it, and the same delegate serves the OpenAccess path | review, wave 3 round 3 | open |
