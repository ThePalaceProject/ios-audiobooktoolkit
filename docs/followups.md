...
| 21 | **`preferChapterEndingHere` is live debt across six call sites.** PP-4948 corrected which chapter a position exactly on a boundary belongs to, but the six resolution sites feeding the `.completed(chapter)` playback signal are pinned to the OLD tie-break, because flipping them changes playback control: `.completed` reaches `handlePlaybackCompleted`, which saves the position and pauses. Measured: without the pin the reported chapter changes for 9 of 10 chapters on `secret_lives_manifest`. The parameter exists to keep the display fix and the playback-control decision independent, and should not outlive that decision — if it does, make it a two-case enum rather than a boolean. Also note `SleepTimer`'s `.completed`-driven end-of-chapter trigger is dead mid-book as a result; the feature still works through its polling fallback (`currentChapter != target` -> sleep), so this is dead code rather than a dead feature | review, PP-4948 | **ticketed (PP-4951)** |
