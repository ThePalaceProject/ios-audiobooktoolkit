...
struct AudiobookNavigationView: View {
  ...
  enum NavigationSection: Identifiable, CaseIterable {
    ...
    var name: String {
      switch self {
      case .toc: DisplayStrings.chapters
      case .bookmarks: DisplayStrings.bookmarks
      }
    }
  }
  ...
  func title(for position: TrackPosition) -> String {
    (try? playback.audiobookManager.audiobook.tableOfContents.chapter(forPosition: position).title) ?? position.track
      .title ?? ""
  }
...
private extension AudiobookNavigationView {
  // Preview init
  ...
  init?() {
    ...
    playback = AudiobookPlaybackModel(audiobookManager: audiobookManager)
    var bookmark = TrackPosition(
      track: audiobook.player.tableOfContents.tracks.first!,
      timestamp: 0.0,
      tracks: audiobook.player.tableOfContents.tracks
    )
...
// MARK: - AudiobookNavigationView_Previews
