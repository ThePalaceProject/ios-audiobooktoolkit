...
class FeedbookDRMProcessor {
  ...
  class func processManifest(_ manifest: [String: Any], drmData: inout [String: Any]) -> Bool {
      ...
      for link in links {
        if (link["rel"] as? String) == "license" {
          if found {
            ATLog(
              .warn,
              "[Feedbook License Status Check] More than one license status link found?! href:\(link["href"] ?? "") type:\(link["type"] ?? "")"
            )
            continue
          }
          found = true
    ...
    guard let signature = metadata
      .removeValue(forKey: "http://www.feedbooks.com/audiobooks/signature") as? [String: Any],
   ...
   */
  private class func verifySignature(_ signatureValue: String, forLicenseDoc: [String: Any]) -> Bool {
    ...
    do {
      ...
      RSAUtils.SHA256HashedData(from: licenseData as NSData).getBytes(
        &digestBytes,
      ...
      URLSession.shared.dataTask(with: licenseCheckUrl) { data, _, error in
           ...
           let statusString = jsonObj["status"] as? String
        {
          if statusString != "ready" && statusString != "active" {
            ...
            weakBook?.applyDRMStatus(.failed)
            return
          }
        }
  ...
  class func getFeedbookSecret(profile: String) -> String {
    ...
    if status == errSecSuccess {
      if item == nil {
        ...
        return String(data: dItem, encoding: .utf8) ?? ""
      } else {
  ...
  private class func getFeedbookPublicKeyFromKeychain(forVendor: String) -> Data? {
  ...
  class func getJWTToken(profile: String, resourceUri: String) -> String? {
    ...
    let signed = preSigned.hmac(
      algorithm: .sha256,
      key: Data(base64Encoded: getFeedbookSecret(profile: profile)) ?? Data()
    ).urlSafeBase64(padding: false)
