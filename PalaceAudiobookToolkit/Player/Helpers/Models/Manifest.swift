...
public extension Manifest {
  ...
  var audiobookType: AudiobookType {
    if let scheme = metadata?.drmInformation?.scheme,
       scheme.contains("http://librarysimplified.org/terms/drm/scheme/FAE")
    {
      return .findaway
    }
  ...
  func profile(for _: AudiobookType) -> String? {
    switch audiobookType {
    case .findaway:
      readingOrder?.first?.properties?.encrypted?.profile
    default:
      nil
    }
  }
