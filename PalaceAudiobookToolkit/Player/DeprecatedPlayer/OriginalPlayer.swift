...
@objc public enum Original_PlaybackRate: Int, CaseIterable {
    ...
    public static func convert(rate: Original_PlaybackRate) -> Float {
        return Float(rate.rawValue) * 0.01
    }
}

/// Receive updates from player as events happen
@objc public protocol Original_PlayerDelegate: class {
...
@objc public protocol OriginalPlayer {
    ...
    var playbackRate: Original_PlaybackRate { get set }
...
extension OriginalPlayer {
    func savePlaybackRate(rate: Original_PlaybackRate) {
        UserDefaults.standard.set(rate.rawValue, forKey: "playback_rate")
    }

    func fetchPlaybackRate() -> Original_PlaybackRate? {
        guard let rate = UserDefaults.standard.value(forKey: "playback_rate") as? Int else { return nil }
        return Original_PlaybackRate(rawValue: rate)
    }
}
