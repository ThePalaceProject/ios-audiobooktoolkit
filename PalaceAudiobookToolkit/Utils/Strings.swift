...
enum Strings {
  ...
  enum AudiobookPlayerViewController {
    ...
    static let playbackDestination = NSLocalizedString(
      "Playback Destination",
      value: "Playback Destination",
      comment: "Describe where the sound can be sent. Example: Bluetooth Speakers."
    )
    static let addBookmark = NSLocalizedString(
      "Add Bookmark",
      value: "Add Bookmark",
      comment: "Button enbaling user to add a bookmark."
    )
    static let addBookmarkAccessiblityHint = NSLocalizedString(
      "Save the current location bookmark for later listening.",
      value: "Save the current location bookmark for later listening.",
      comment: "Explain how to set a bookmark."
    )
    static let bookmarkAdded = NSLocalizedString(
      "Bookmark added",
    ...
    static let destinationAvailabilityAccessiblityHint = NSLocalizedString(
      "If another device is available, send the audio over Bluetooth or Airplay. Otherwise do nothing.",
      value: "If another device is available, send the audio over Bluetooth or Airplay. Otherwise do nothing.",
      comment: "Longer description to describe action of the button."
    )
    static let timeToPause = NSLocalizedString(
      "%@ until playback pauses",
      value: "%@ until playback pauses",
      comment: "localized time until playback pauses, for voice over"
    )
    static let trackAt = NSLocalizedString("Track %d", value: "Track %d", comment: "Default track title")
    static let fileNumber = NSLocalizedString(
      "%@ (file %@ of %d)",
      value: "%@ (file %@ of %d)",
      comment: "Current chapter and the amount of chapters left in the book"
    )
    static let problemHasOccurred = NSLocalizedString(
  ...
  enum AudiobookTableOfContentsTableViewController {
  ...
  enum Generic {
    ...
    static let downloadingFormatted = NSLocalizedString(
      "Downloading: %@%%",
      value: "Downloading: %@%%",
      comment: "The percentage of the chapter that has been downloaded, formatting for string should be localized at this point."
    )
  ...
  enum Error {
    static let bookmarkAlreadyExistsError = NSLocalizedString(
      "A bookmark has already been saved at this location.",
      comment: "Alert notifying user that location has already been saved."
    )
    static let failedToSaveBookmarkError = NSLocalizedString(
      "Internal Error, bookmark failed to save.",
      comment: "Alert notifying user that bookmark save failed."
    )
  }

  enum PlaybackControlView {
    static let rewind = NSLocalizedString(
      "Rewind %d seconds",
      value: "Rewind %d seconds",
      comment: "Rewind a configurable number of seconds"
    )
    static let forward = NSLocalizedString(
      "Fast Forward %d seconds",
      value: "Fast Forward %d seconds",
      comment: "Fast forward a configurable number of seconds"
    )
  }
  ...
  enum ScrubberView {
    static let timeRemaining = NSLocalizedString(
      "%@ remaining",
      value: "%@ remaining",
      comment: "The amount of hours and minutes left"
    )
    static let timeRemainingInBook = NSLocalizedString(
      "%@ remaining in the book.",
      value: "%@ remaining in the book.",
      comment: "How much time is left in the entire book, not just the chapter."
    )
    static let playedVsRemaining = NSLocalizedString(
      "%@ played. %@ remaining.",
      value: "%@ played. %@ remaining.",
      comment: "Time into the current chapter, then time remaining in the current chapter."
    )
    static let downloading = NSLocalizedString(
      "Downloading...",
      value: "Downloading...",
      comment: "Downloading..."
    )
    static let playbackSliderValueDescription = NSLocalizedString(
      "Playback progress %.2f percent",
